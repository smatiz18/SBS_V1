...
cat "$TEMP_FILE" >> ~/.cron_profile

# Remove the temporary decrypted file
rm -f "$TEMP_FILE"

echo "Environment variables have been successfully set for ~/.cron_profile ."

echo "ensure you activate ~/.cron_profile when setting up cron jobs"
