...
pub async fn get_nba_games_by_team_and_season(collection: &Collection<Document>, teamId: f64, season: f64) -> Result<Vec<NbaGamesHistorical>> {
   let query = doc! { 
        "$or": [ 
            { "teamsHomeId": teamId }, 
            { "teamsVisitorsId": teamId }
        ],
        "season": season
    };
